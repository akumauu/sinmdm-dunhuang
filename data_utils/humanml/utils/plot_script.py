...
def explicit_plot_3d_motion(save_path, kinematic_tree, joints, title, dataset, figsize=(3, 3), fps=120, radius=3, vis_mode="default", frame_colors=[]):
    ...
    if dataset == "humanml":
        ...
        data *= 1.0  # Reduced 20x as per user request (was 20)
    ...
    # Recalculate BBox after centering
    ...
    def update(index):
        ...
        # Re-apply limits and background
        ax.set_xlim3d([center_centered[0] - max_range_centered, center_centered[0] + max_range_centered])
        ax.set_ylim3d([center_centered[1] - max_range_centered, center_centered[1] + max_range_centered])
        ax.set_zlim3d([center_centered[2] - max_range_centered, center_centered[2] + max_range_centered])
        ...
        # Adjust xzPlane to follow trajectory if needed, but since we center it, we can just use fixed ground
        plot_xzPlane(MINS_centered[0], MAXS_centered[0], 0, MINS_centered[2], MAXS_centered[2])
