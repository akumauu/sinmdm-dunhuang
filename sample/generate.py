...
def main(args=None):
    ...
    if args.dataset == 'mixamo':
        ...
        fps = 20
        # For SinMDM with humanml dataset:
        # 1. First check if args.num_joints is already set (by train_sinmdm.py or generate_during_training)
        # 2. Otherwise, infer from checkpoint's input_proj.weight
        if hasattr(args, 'num_joints') and args.num_joints and args.num_joints != '':
            num_joints = int(args.num_joints)
            print(f'Using num_joints={num_joints} from args.')
        elif args.model_path and os.path.exists(args.model_path):
            state_dict = torch.load(args.model_path, map_location='cpu')
            if 'input_proj.weight' in state_dict:
                num_joints = state_dict['input_proj.weight'].shape[1]
                print(f'Inferred num_joints={num_joints} from checkpoint.')
            else:
                num_joints = None
        else:
            num_joints = None
    ...
    if args.dataset in ['humanml']:
        # For single motion training (SinMDM), the full HumanML3D dataset may not be present.
        # We check for Mean.npy which is required by the HumanML3D dataset class.
        mean_path = os.path.join(args.data_dir if args.data_dir else './dataset/HumanML3D', 'Mean.npy')
        if os.path.exists(mean_path):
            print('Loading dataset...')
            data = load_dataset(args, max_frames, n_frames)
        else:
            print('Warning: Mean.npy not found. Skipping HumanML3D dataset loading. '
                  'Visualization may be inaccurate if data_rep is hml_vec.')
            data = None
    else:
        data = None
