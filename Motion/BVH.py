...
def save(filename, anim, names=None, frametime=1.0/24.0, order='xyz', positions=False, orients=True):
    ...
    with open(filename, 'w') as f:
        ...
        rots = np.unwrap(np.deg2rad(rots), axis=0)
        rots = np.rad2deg(rots)
        
        poss = anim.positions
        
        # Calculate expected number of columns
        # ROOT (6) + (Number of non-root joints * 3 if positions=False else 6)
        # Assuming end sites are handled in save_joint but not in MOTION data block for some joints?
        # Actually, the logic below writes data for all joints EXCEPT end_sites.
        num_non_root_joints = 0
        for j in range(1, anim.shape[1]):
            if j not in end_sites:
                num_non_root_joints += 1
        
        expected_cols = 6 + (3 if not positions else 6) * num_non_root_joints
        ...
        for i in range(anim.shape[0]):
            ...
            for j in range(anim.shape[1]):
                if j not in end_sites:
                    if j == 0: # ROOT
                        frame_data.extend([poss[i,j,0], poss[i,j,1], poss[i,j,2],
                                         rots[i,j,ordermap[print_order[0]]], rots[i,j,ordermap[print_order[1]]], rots[i,j,ordermap[print_order[2]]]])
                    else: # JOINT
                        if positions:
                            frame_data.extend([poss[i,j,0], poss[i,j,1], poss[i,j,2]])
                        frame_data.extend([rots[i,j,ordermap[print_order[0]]], rots[i,j,ordermap[print_order[1]]], rots[i,j,ordermap[print_order[2]]]])

            assert len(frame_data) == expected_cols, f"Column count mismatch: expected {expected_cols}, got {len(frame_data)}"
            f.write(" ".join([f"{val:f}" for val in frame_data]) + " \n")
